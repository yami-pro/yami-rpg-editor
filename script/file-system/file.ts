...
interface File {
  ...
  get(descriptor: Type.descriptor): Promise<HTMLImageElement | null>
...
File.getImageResolution = function IIFE() {
  const promises: {[key: string]: Promise<Type.resolution>} = {}
  const resolution: Type.resolution = {width: 0, height: 0}
