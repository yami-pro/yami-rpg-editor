...
class FileBodyPane extends HTMLElement {
  ...
  getFiles() {
    ...
    if (!filters) {
      ...
      for (const folder of folders) {
        length += (<FolderItem>folder).children.length
      }
      let i = 0
      const items = new Array(length)
      for (const folder of folders) {
        for (const item of (<FolderItem>folder).children) {
          items[i++] = item
        }
      }
      return items
    ...
    for (const folder of folders) {
      for (const item of (<FolderItem>folder).children) {
        // FileItem 没有children属性 use instanceof?
  ...
  updateFiles() {
    ...
    // 创建列表项目
    const browser = this.links.browser
  ...
  // 创建扁平排列的项目
  createFlatItems(dir: (FileItem | FolderItem)[]) {
  ...
  // 选择项目
  select(...files: (FolderItem | FileItem)[]) {
    this.unselect()
  ...
  // 在网格列表中选择相对位置的项目
  selectRelativeInGridMode(direction: string) {
    const {elements} = this
    const {count} = elements
    if (count > 0) {
      let index
    ...
    if (selections.length === 1 && this.hasScrollBar()) {
      ...
      for (let i = 0; i < count; i++) {
        if (elements[i].file === selection) {
          ...
          let scroll: number = this[property]
          switch (mode) {
            case 'active':
              scroll = Math.clamp(
                scroll,
  ...
  // 打开文件
  openFile(file: FolderItem | FileItem) {
    if (file instanceof FolderItem) {
      const {nav} = this.links
      nav.load(file)
  ...
  // 删除文件
  deleteFiles() {
    const files: (FolderItem | FileItem)[] = []
    const {selections} = this
  ...
  // 重命名
  rename(file: FolderItem | FileItem) {
  ...
  pointerdown(this: FileBodyPane, event: PointerEvent) {
    ...
    switch (event.button) {
      case 0: case 2: {
        let element = <FileBodyPane>event.target
        ...
        if (element === this) {
          ...
          if (element.tagName === 'FILE-BODY-ICON' ||
            ...
            element = <FileBodyPane>element.parentNode
          }
            ...
            if (!element.hasClass('selected')) {
              ...
              if (length > 1) {
                ...
                window.on('pointerup', pointerup, {once: true})
                          ...
                          event !== null &&
                          event.target !== null &&
                          (<HTMLElement>event.target).tagName === 'FILE-BODY-NAME') {
  ...
  static textBox = function IIFE() {
    ...
    textBox.on('keydown', function (this: TextBox, event: KeyboardEvent) {
      ...
      switch (event.code) {
        ...
        case 'Escape': {
          ...
          if (item !== null) {
            const content = <HTMLElement>item.parentNode
            this.input.blur()
            content.focus()
          }
          break
    ...
    // 输入前事件
    textBox.on('beforeinput', function (event: InputEvent) {
    ...
    // 输入事件
    textBox.on('input', function (this: TextBox, event: InputEvent) {
      if (this.style.width !== '') {
        this.fitContent()
      }
    })
    ...
    // 失去焦点事件
    textBox.on('blur', function (this: TextBox, event: InputEvent) {
      const item = <FileBodyPane>this.parentNode
