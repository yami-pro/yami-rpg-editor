...
interface Editor {
  ...
  switchHotkey: (enabled: boolean) => void
  protectPromise(promise: Promise<void>): Promise<void>
  saveConfig(): void
  loadConfig(): void
  saveProject(): void
  loadProject(): void
...
Editor.switchHotkey = function IIFE() {
  const keydown = function (event: Event) {
...
// 保护承诺对象
Editor.protectPromise = function (promise: Promise<void>) {
  const {promises} = this
